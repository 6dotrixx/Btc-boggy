...
function update(dt) {
  ...
  if (mv.moving) {
    ...
    // footfall: each stride kicks up a little deck dust
    const stepSign = Math.sign(Math.sin(player.walkT));
    if (stepSign !== player._stepSign) {
      player._stepSign = stepSign;
      if (player.thrust > 0.4) {
        for (let i = 0; i < 2; i++) {
          particles.push({ x: player.x + rand(-5, 5), y: player.y + player.r * 0.85,
            vx: rand(-20, 20) - mv.x * 45, vy: rand(-14, -4) - mv.y * 20,
            life: rand(0.22, 0.38), color: '#9397ab', r: rand(1.5, 3) });
        }
      }
    }
  } else {
...
function drawMonster(e) {
  ...
  if (img) {
    ...
    // walkers stomp (hop + gait rock); hover units drift by design
    const dy = e.shape === 'walker' ? -Math.abs(Math.sin(e.wobble * 2)) * 2.5 : Math.sin(e.wobble) * 1.5;
    ctx.save(); ctx.translate(e.x, e.y + dy);
    if (e.shape === 'walker') ctx.rotate(ea0 + Math.PI / 2 + Math.sin(e.wobble * 2) * 0.07);
    else if (e.shape === 'tank' || e.shape === 'gunner') ctx.rotate(ea0 + Math.PI / 2);
...
function drawShip() {   // draws the ranger (name kept for call sites)
  ...
  // real character art when available — with a proper walk cycle so steps read
  const img = heroSpriteImg();
  if (img) {
    softShadow(player.x, player.y + 4, r);                    // shadow stays grounded
    const wt = player.walkT || 0;
    const hop = run * Math.abs(Math.sin(wt)) * 5;             // lift on each stride
    const tilt = run * Math.sin(wt) * 0.09;                   // lean into the step
    const squash = 1 - run * 0.05 * Math.abs(Math.cos(wt));   // contact squash at footfall
    ctx.save(); ctx.translate(player.x, player.y - hop);
